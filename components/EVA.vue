<template>
  <div class="text-center">
    <h1 class="text-4xl text-heading font-bold tracking-wide">
      Introducing E.V.A
    </h1>

    <div class="EVA flex flex-wrap justify-center items-center my-10 gap-16 mx-10">
      <div
        class="inline-block w-96 text-center p-8"
      >
      <div class="flex justify-center items-center mb-2">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="40"
          height="40"
          viewBox="0 0 640 512"
        >
          <path
            fill="#ffbb00"
            d="M624 352h-16V243.9c0-12.7-5.1-24.9-14.1-33.9L494 110.1c-9-9-21.2-14.1-33.9-14.1H416V48c0-26.5-21.5-48-48-48H112C85.5 0 64 21.5 64 48v48H8c-4.4 0-8 3.6-8 8v16c0 4.4 3.6 8 8 8h272c4.4 0 8 3.6 8 8v16c0 4.4-3.6 8-8 8H40c-4.4 0-8 3.6-8 8v16c0 4.4 3.6 8 8 8h208c4.4 0 8 3.6 8 8v16c0 4.4-3.6 8-8 8H8c-4.4 0-8 3.6-8 8v16c0 4.4 3.6 8 8 8h208c4.4 0 8 3.6 8 8v16c0 4.4-3.6 8-8 8H64v128c0 53 43 96 96 96s96-43 96-96h128c0 53 43 96 96 96s96-43 96-96h48c8.8 0 16-7.2 16-16v-32c0-8.8-7.2-16-16-16M160 464c-26.5 0-48-21.5-48-48s21.5-48 48-48s48 21.5 48 48s-21.5 48-48 48m320 0c-26.5 0-48-21.5-48-48s21.5-48 48-48s48 21.5 48 48s-21.5 48-48 48m80-208H416V144h44.1l99.9 99.9z"
          />
        </svg>
      </div>
        

        <h1 class="text-heading font-semibold text-xl">Self Driving</h1>
        <p class="text-[#67625f]">
          EVA has the functionality to drive your car on auto-pilot to define
          loaction
        </p>
      </div>

      <div class="w-96 text-center content-center p-8">
        <div class="flex justify-center items-center mb-2">
           <svg
          xmlns="http://www.w3.org/2000/svg"
          width="28"
          height="32"
          viewBox="0 0 448 512"
        >
          <path
            fill="#ffbb00"
            d="M448 96v256c0 51.815-61.624 96-130.022 96l62.98 49.721C386.905 502.417 383.562 512 376 512H72c-7.578 0-10.892-9.594-4.957-14.279L130.022 448C61.82 448 0 403.954 0 352V96C0 42.981 64 0 128 0h192c65 0 128 42.981 128 96m-48 136V120c0-13.255-10.745-24-24-24H72c-13.255 0-24 10.745-24 24v112c0 13.255 10.745 24 24 24h304c13.255 0 24-10.745 24-24m-176 64c-30.928 0-56 25.072-56 56s25.072 56 56 56s56-25.072 56-56s-25.072-56-56-56"
          />
        </svg>
        </div>
       
        <h1 class="text-heading font-semibold text-xl">Car Monitoring</h1>

        <p class="text-[#67625f]">
          EVA monitors the health of your car to let you know faults parts and
          issues.
        </p>
      </div>

      <div class="w-96 text-center content-center p-8">
        <div class="flex justify-center items-center mb-2">
          <svg
          xmlns="http://www.w3.org/2000/svg"
          width="36"
          height="32"
          viewBox="0 0 576 512"
        >
          <path
            fill="#ffbb00"
            d="M288 0c-69.59 0-126 56.41-126 126c0 56.26 82.35 158.8 113.9 196.02c6.39 7.54 17.82 7.54 24.2 0C331.65 284.8 414 182.26 414 126C414 56.41 357.59 0 288 0m0 168c-23.2 0-42-18.8-42-42s18.8-42 42-42s42 18.8 42 42s-18.8 42-42 42M20.12 215.95A32.006 32.006 0 0 0 0 245.66v250.32c0 11.32 11.43 19.06 21.94 14.86L160 448V214.92c-8.84-15.98-16.07-31.54-21.25-46.42zM288 359.67c-14.07 0-27.38-6.18-36.51-16.96c-19.66-23.2-40.57-49.62-59.49-76.72v182l192 64V266c-18.92 27.09-39.82 53.52-59.49 76.72c-9.13 10.77-22.44 16.95-36.51 16.95m266.06-198.51L416 224v288l139.88-55.95A31.996 31.996 0 0 0 576 426.34V176.02c0-11.32-11.43-19.06-21.94-14.86"
          />
        </svg>
        </div>
        
        <h1 class="text-heading font-semibold text-xl">Car Tracking</h1>

        <p class="text-[#67625f]">
          EVA tracks the location of your car for security with smart alerts
        </p>
      </div>

      <div class="w-96 text-center content-center p-8">
        <div class="flex justify-center items-center mb-2">
          <svg
          xmlns="http://www.w3.org/2000/svg"
          width="36"
          height="32"
          viewBox="0 0 576 512"
        >
          <path
            fill="#ffbb00"
            d="M0 117.66v346.32c0 11.32 11.43 19.06 21.94 14.86L160 416V32L20.12 87.95A32.006 32.006 0 0 0 0 117.66M192 416l192 64V96L192 32zM554.06 33.16L416 96v384l139.88-55.95A31.996 31.996 0 0 0 576 394.34V48.02c0-11.32-11.43-19.06-21.94-14.86"
          />
        </svg>
        </div>
        
        <h1 class="text-heading font-semibold text-xl">Traveling Assistant</h1>

        <p class="text-[#67625f]">
          EVA scans the map for facilities such as charging stations, repair
          pont
        </p>
      </div>

      <div class="w-96 text-center content-center p-8">
        <div class="flex justify-center items-center mb-2">
          <svg
          xmlns="http://www.w3.org/2000/svg"
          width="30"
          height="32"
          viewBox="0 0 480 512"
        >
          <path
            fill="#ffbb00"
            d="m438.66 212.33l-11.24-28.1l-19.93-49.83C390.38 91.63 349.57 64 303.5 64h-127c-46.06 0-86.88 27.63-103.99 70.4l-19.93 49.83l-11.24 28.1C17.22 221.5 0 244.66 0 272v48c0 16.12 6.16 30.67 16 41.93V416c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32v-32h256v32c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32v-54.07c9.84-11.25 16-25.8 16-41.93v-48c0-27.34-17.22-50.5-41.34-59.67m-306.73-54.16c7.29-18.22 24.94-30.17 44.57-30.17h127c19.63 0 37.28 11.95 44.57 30.17L368 208H112zM80 319.8c-19.2 0-32-12.76-32-31.9S60.8 256 80 256s48 28.71 48 47.85s-28.8 15.95-48 15.95m320 0c-19.2 0-48 3.19-48-15.95S380.8 256 400 256s32 12.76 32 31.9s-12.8 31.9-32 31.9"
          />
        </svg>
        </div>
        
        <h1 class="text-heading font-semibold text-xl">Co-pilot</h1>

        <p class="text-[#67625f]">
          EVA anbales you to access your phone and systems through voice
          command.
        </p>
      </div>
    </div>
  </div>
</template>

<script setup></script>

<style lang="scss" scoped></style>
